          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
            {mainFeatures.map((feature, index) => (
              <Link
                key={index}
                href={feature.href}
                className={`group relative p-4 bg-bg-secondary rounded-xl border border-border hover:border-accent/50 transition-all duration-300 hover:translate-y-[-4px] hover:shadow-lg hover:shadow-accent/10 text-center ${isLoaded ? 'animate-fade-in-up' : 'opacity-0'}`}
                style={{ animationDelay: `${600 + index * 50}ms` }}
              >
                <div className={`w-12 h-12 mx-auto rounded-xl bg-gradient-to-br ${feature.color} flex items-center justify-center text-white mb-3 group-hover:scale-110 transition-transform duration-300 shadow-lg`}>
                  {feature.icon}
                </div>
            ...
            >
              ...
              <div className="flex-1 text-center lg:text-left">
                <div
                  ...
                  style={{
                    background: `linear-gradient(to right, ${feature.gradient.includes("orange") ? "rgba(249, 115, 22, 0.1)" : feature.gradient.includes("red") ? "rgba(239, 68, 68, 0.1)" : feature.gradient.includes("yellow") ? "rgba(234, 179, 8, 0.1)" : feature.gradient.includes("violet") ? "rgba(139, 92, 246, 0.1)" : feature.gradient.includes("blue") ? "rgba(59, 130, 246, 0.1)" : "rgba(16, 185, 129, 0.1)"}, transparent)`,
                  }}
                >
