...
export default function Home() {
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative py-20 px-4 overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto text-center relative">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link
              href="/crafting"
              className="px-8 py-4 bg-accent text-white rounded-xl font-semibold hover:bg-accent-hover transition-all hover:scale-105 shadow-lg shadow-accent/25"
            >
              Open Crafting Calculator
            </Link>
            <Link
              href="/register"
              className="px-8 py-4 bg-bg-secondary text-text-primary rounded-xl font-semibold hover:bg-bg-hover transition-all border border-border hover:border-accent"
            >
              Join the Community
            </Link>
          </div>
      ...
      {/* Stats Bar */}
      <section className="py-8 px-4 bg-bg-secondary border-y border-border">
        <div className="max-w-5xl mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div className="text-center">
              <div className="text-3xl md:text-4xl font-bold text-accent">{stats.items}</div>
              <div className="text-sm text-text-muted mt-1">Items in Database</div>
            </div>
            <div className="text-center">
              <div className="text-3xl md:text-4xl font-bold text-accent">{stats.recipes}</div>
              <div className="text-sm text-text-muted mt-1">Crafting Recipes</div>
            </div>
            <div className="text-center">
              <div className="text-3xl md:text-4xl font-bold text-accent">{stats.members}</div>
              <div className="text-sm text-text-muted mt-1">Community Members</div>
            </div>
            <div className="text-center">
              <div className="text-3xl md:text-4xl font-bold text-accent">{stats.orders}</div>
              <div className="text-sm text-text-muted mt-1">Active Orders</div>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {mainFeatures.map((feature, index) => (
              <Link
                key={index}
                href={feature.href}
                className="group relative p-6 bg-bg-secondary rounded-2xl border border-border hover:border-accent/50 transition-all duration-300 overflow-hidden"
              >
                ...
                <div className="relative">
                  ...
                  <div className="mt-4 flex items-center gap-2 text-accent font-medium text-sm opacity-0 group-hover:opacity-100 transition-opacity">
                    <span>Explore</span>
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {additionalFeatures.map((feature, index) => (
              <Link
                key={index}
                href={feature.href}
                className="flex items-start gap-4 p-4 rounded-xl bg-bg-primary border border-border hover:border-accent/50 transition-colors group"
              >
      ...
      <section className="py-20 px-4">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <Link
            href="/crafting"
            className="inline-flex items-center gap-2 px-8 py-4 bg-accent text-white rounded-xl font-semibold hover:bg-accent-hover transition-all hover:scale-105 shadow-lg shadow-accent/25"
          >
            Open Crafting Calculator
